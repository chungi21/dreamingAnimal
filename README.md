# 개인 프로젝트 | Dreaming Animal - 위치 기반 보호동물 커뮤니티 플랫폼(Spring Boot · JPA · Security · OAuth2.0 · Kakao API )
...
## 후기
이번 프로젝트는 독학 후 처음으로 진행한 개인 웹 프로젝트로, 기획 단계부터 구현까지 직접 경험하며 많은 배움을 얻을 수 있었습니다.<br/>
처음에는 부족한 부분이 많아 기능을 수정·추가하는 과정을 반복했고, 이 과정에서 기획과 구현의 균형을 맞추는 중요성을 체감했습니다.<br/>
<br/>
개발 과정에서는 단순히 기능을 구현하는 것에 그치지 않고, 왜 이 방식이 맞는지를 스스로 고민하며 선택하려고 노력했습니다. 예를 들어, 상황에 따라 GET/POST를 어떻게 구분해야 하는지, @Controller와 @RestController의 차이를 어디서 적용해야 하는지, DTO와 Entity를 어떤 시점에 변환해야 하는지 등 다양한 지점을 깊이 있게 고민했습니다. 이러한 과정은 단순한 트렌드 습득을 넘어, 그 원리를 이해하고 제 코드에 적용하는 계기가 되었습니다.<br/><br/>

이번 프로젝트를 통해 아직 배워야 할 점이 많다는 것을 깨달았지만, 동시에 스스로 부족한 부분을 인식하고 앞으로 나아갈 방향성을 찾았다는 점에서 큰 의미가 있습니다. 앞으로도 꾸준한 경험과 학습을 통해 더 나은 개발자로 성장해 나가겠습니다.<br/>
